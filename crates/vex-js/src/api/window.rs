...
pub fn register(context: &mut Context) {
    ...
    let window = ObjectInitializer::new(context)
        .property(js_string!("location"), location, Attribute::WRITABLE | Attribute::CONFIGURABLE)
        ...
        .function(NativeFunction::from_fn_ptr(alert_fn), js_string!("alert"), 1)
        .build();

    // Register `window` as a global property.
    context
        .register_global_property(js_string!("window"), window.clone(), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .expect("failed to register window global");

    // `self` === `window` in browsers.
    context
        .register_global_property(js_string!("self"), window, Attribute::WRITABLE | Attribute::CONFIGURABLE)
    ...
    // Top-level `alert()`.
    context
        .register_global_callable(js_string!("alert"), 1, NativeFunction::from_fn_ptr(alert_fn))
...
fn build_location(context: &mut Context) -> JsValue {
    ObjectInitializer::new(context)
        .property(js_string!("href"), js_string!("about:blank"), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .property(js_string!("origin"), js_string!("null"), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .property(js_string!("protocol"), js_string!("about:"), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .property(js_string!("pathname"), js_string!("blank"), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .property(js_string!("search"), js_string!(""), Attribute::WRITABLE | Attribute::CONFIGURABLE)
        .property(js_string!("hash"), js_string!(""), Attribute::WRITABLE | Attribute::CONFIGURABLE)
...
fn build_history(context: &mut Context) -> JsValue {
    ObjectInitializer::new(context)
        .property(js_string!("length"), JsValue::from(1), Attribute::CONFIGURABLE)
...
fn history_forward(_this: &JsValue, _args: &[JsValue], _context: &mut Context) -> JsResult<JsValue> {
    tracing::info!("history.forward()");
    Ok(JsValue::undefined())
}

fn history_push_state(_this: &JsValue, args: &[JsValue], context: &mut Context) -> JsResult<JsValue> {
    ...
    if !url.is_undefined() && !url.is_null() {
        ...
        tracing::info!("history.pushState(url=\"{}\")", url_str.to_std_string_escaped());
    } else {
...
fn build_navigator(context: &mut Context) -> JsValue {
    ...
    ObjectInitializer::new(context)
        ...
        .property(js_string!("appName"), js_string!("Vex"), Attribute::CONFIGURABLE)
        .property(js_string!("platform"), platform_string(), Attribute::CONFIGURABLE)
        .property(js_string!("language"), js_string!("en-US"), Attribute::CONFIGURABLE)
        .property(js_string!("cookieEnabled"), JsValue::from(true), Attribute::CONFIGURABLE)
        .build()
        .into()
}
